...
geoescapes_3 <- read.csv("C:/Users/keyserf/Documents/Data/R output/NL Atlantic Salmon Escape Events.csv")
...
avg.escape.per.year <- ddply(.data=geoescapes_3, .(Location),
      summarize,
...
avg.escape.size <- ddply(.data=geoescapes_3[is.na(geoescapes_3$Number.adj)==FALSE,], .(Location),
      summarize,
      totalescape = sum(Number.adj),
      nyears = length(unique(Year)),
      average=sum(Number.adj)/length(unique(Year)))

summary(geoescapes_3$Number.adj)

mean(geoescapes_3$Number.adj, na.rm=TRUE)

# If escape size is unknown, assume that it is equal to average escape size
geoescapes_3$escapesize <- ifelse(is.na(geoescapes_3$Number.adj)=="TRUE", "26432", geoescapes_3$Number.adj)

geoescapes_3$escapesize <- as.numeric(geoescapes_3$escapesize)

geoescapes_3$escapesizetype <- is.na(geoescapes_3$Number.adj)
...
### ALL YEARS
escape_summ_3 <- ddply(.data=geoescapes_3, .(Location, Lat, Long, Year),
                     summarize,
                    totalescape = sum(escapesize))

escape_summ_3$Location <- as.factor(escape_summ_3$Location)

#### USE ESCAPE_SUMM_3 TO CALCULATE ESCAPE PRESSURE IN PROPAGULE PRESSURE.RMD
