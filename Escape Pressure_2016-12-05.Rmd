...
geoescapes_2 <- dplyr::select(geoescapes, Year, Location, Lat, Long, Number.adj, Size.adj, Size.bin, Other.Observations)
...
# Q values AQ escape site coords
x <- escape_summ$Long
y <- escape_summ$Lat
xy <- cbind(x, y)
siterast <- as.matrix(xy)
...
# for q values
# plot(NLprovrast, xlim=c(-56.5, -55.5), ylim=c(47.5, 48.3))
# text(siterast[,1], siterast[,2])
# siterast[12,] <- c(-55.78495, 47.57846)
...
write.csv(esc.press_all, "C:/Users/keyserf/Documents/Data/Escape Pressure_lin_all rivers_all years_2016-12-05.csv")
...
# sum all pressures for total propagule pressure at a river
esc.press_q <- apply(as.matrix(dist2Escape[,20:34]), 2, function(x) sum(x, na.rm=TRUE))

# match to river mouth coords
esc.press_q  <- data.frame(code=qsites$Code, lat=qsites[,6], long = qsites[,5], esc.press = esc.press_q, q=qsites$Q)

write.csv(esc.press_q, "C:/Users/keyserf/Documents/Data/Escape Pressure_NL_Q values_2002-2012_2016-12-05.csv")
...
plot2 <- ggplot() + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank()) +
  scale_size_continuous(name="Escape pressure=\n\u03A3(total escapes/dist)", range = c(0.5,8), limits=c(1,180))
...
CPUE <- join(catch, effort, type="left")

CPUE$adj.catch <- CPUE$catch/as.numeric(CPUE$time)


### Compare against propagule pressure
prop.press_NL <- read.csv("C:/Users/keyserf/Documents/Data/R output/Propagule Pressure_NL_all years_2016-12-01.csv")
plot(NLrast, xlim=c(-56, -55), ylim=c(47.5, 48))
text(CPUE$long, CPUE$lat)
CPUE$code <- as.factor(c("FBN", "SMB", "FBN", "FBN"))

test <- join(CPUE[,c(1,2,3,6,7,8)], prop.press_NL, type="left")

ggplot() + geom_point(data=test, aes(prop.press, adj.catch))
...
### New Brunswick - Read in Geoff's escape reports db and tidy up
```{r}
...
```{r}
### ALL YEARS
escape_summ_2 <- ddply(.data=NBescapes, .(ID, Lat, Long),
                     summarize,
                    totalescape = sum(escapesize))
...
escape_summ <- ddply(.data=NBescapes2, .(ID, Lat, Long),
                     summarize,
                     totalescape = sum(escapesize))
```
...
plot2 <- ggplot() + 
  ...
  scale_size_continuous(name="Escape pressure=\n\u03A3(total escapes/dist)", range = c(0.5,8), limits=c(1,8))
