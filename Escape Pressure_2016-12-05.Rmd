...
geoescapes_sal[]<- sapply(geoescapes_sal, clean)
...
geoescapes_2$escapesize <- ifelse(is.na(geoescapes_2$Number.adj)=="TRUE", "10375", geoescapes_2$Number.adj)
...
```{r}
### NOT REQUIRED FOR ATL SALMON ONLY
...
plot2 <- ggplot() + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank()) +
  scale_size_continuous(name="Escape pressure=\n\u03A3(total escapes/dist)", range = c(0,4), limits=c(0,4))
