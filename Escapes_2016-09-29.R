...
extent(NLline)
NLline <- crop(NLline, extent(-57, -54.5, 47, 48))

r <- raster(ncol=1800, nrow=1550)
extent(r) <- extent(NLline)
NLlinerast <- rasterize(NLline, r, fun="first")
plot(NLlinerast)
NLlinerast@data@values <- ifelse(is.na(NLlinerast@data@values)=="TRUE", NA, 10000)
...
NLpoints <- data.frame(shortest.col = 1:28590, point.lat = NLpoints$y, point.long = NLpoints$x)
...
ggplot() + geom_point(data=data.frame(NLpoints), aes(x, y), size=0.5) + 
  geom_point(data=sites, aes(Long, Lat), colour="red") +
  coord_map()
