...
### FOR SAMPLING DATA (HIGH RES, SMALL AREA)

# require(rcanvec)
# 
# nts(lon=-55.5, lat=47.6)
# #001M11
# canvec.download(nts('001M11'))
# canvec1 <- canvec.load(nts('001M11'), "waterbody")
# plot(canvec1)
# 
# nts(lon=-55.75, lat=47.9)
# #001M13
# canvec.download(nts('001M13'))
# canvec2 <- canvec.load(nts('001M13'), "waterbody")
# plot(canvec2, add=TRUE)
# 
# nts(lon=-55.5, lat=47.9)
# #001M14
# canvec.download(nts('001M14'))
# canvec3 <- canvec.load(nts('001M14'), "waterbody")
# plot(canvec3, add=TRUE)
# 
# nts(lon=-55.75, lat=47.6)
# #001M12
# canvec.download(nts('001M12'))
# canvec4 <- canvec.load(nts('001M12'), "waterbody")
# plot(canvec4, add=TRUE)
# 
# plot(NLrast, add=TRUE)
...
escapelocs <- unique(select(escapes, Waypoint.ID, Latitude..N., Longitude..E.))
...
NLlinerast@data@values <- ifelse(is.na(NLlinerast@data@values)=="TRUE", 1, 10000)
```
...
tr3 <- transition(1/NLlinerast, transitionFunction=mean, directions=8)
...
d2 <- costDistance(tr3c, fromCoords = siterast, toCoords = escapemat)
d2 

shortest.dist <- apply(d2, 2, min)
shortest.col <- apply(d2, 2, which.min)

sitedist <- data.frame(lat=escapelocs$Latitude..N., long = escapelocs$Longitude..E., shortest.dist = shortest.dist, shortest.col = shortest.col)

sitelist <- data.frame(shortest.col = 1:49, siteLat = sites$Lat, siteLong = sites$Long)

sitedist.join <- join(sitedist, sitelist, type="left")
...
plot(NLlinerast, xlim=c(-55.5, -55.25), ylim=c(47.6, 47.8))
lines(sPath)
