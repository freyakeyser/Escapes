...
plot(NLrast)
points(escapelocs$Longitude..E., escapelocs$Latitude..N.)
lines(NLline)
# rivermouths <- click(n=3)
```
...
NLrast@data@values <- ifelse(NLrast@data@values=="10000", 100000, 1)
```
...
d3 <- costDistance(tr3c, fromCoords = siterast, toCoords = rivermouths)
d3 

shortest.dist <- apply(d3, 2, min)
shortest.col <- apply(d3, 2, which.min)
