...
riverCWD <- data.frame(code=Bsites[,1], lat=rivermouths[,2], long = rivermouths[,1], prop.press = prop.press_all)
...
### Annual propagule pressure for each river
```{r}
...
### Total propagule pressure for ALL Newfoundland rivers
```{r}
...
names(qsites) <- c("code", "dist", "q", "long", "lat")

qsites <- qsites[!is.na(qsites$lat),]

# nudge AQ site record #37 into water
plot(NLprovrasttest, xlim=c(-56.2, -55), ylim=c(47.6, 48))
points(siterast[37,1], siterast[37,2], col="red")
siterast[37,] <- c(-55.15866, 47.63114)
plot(NLprovrasttest, xlim=c(-56.5, -55), ylim=c(47, 48.2))
points(siterast[21,1], siterast[21,2], col="black")
siterast[21,] <-c(-56.12302, 47.74039)
siterast[34,] <- c(-56.12969, 47.70439)
...
colnames(d4)[12] <- "ID"
colnames(d4)[1:11] <- as.list(as.character(qsites$code))
d4 <- as.data.frame(d4)

dist2AQ_prov <- join(d4, totfishall, type="left")

dist2AQ_prov[,1:11] <- apply(dist2AQ_prov[,1:11], 2, function(x) as.character(x))
dist2AQ_prov[,1:11] <- apply(dist2AQ_prov[,1:11], 2, function(x) as.numeric(x))
...
dist2AQ_prov$P7 <- dist2AQ_prov$totalfish/dist2AQ_prov$RKR
dist2AQ_prov$P8 <- dist2AQ_prov$totalfish/dist2AQ_prov$NBT
dist2AQ_prov$P9 <- dist2AQ_prov$totalfish/dist2AQ_prov$SGR
dist2AQ_prov$P10 <- dist2AQ_prov$totalfish/dist2AQ_prov$WAB
dist2AQ_prov$P11 <- dist2AQ_prov$totalfish/dist2AQ_prov$CMP

# sum all pressures for total propagule pressure at a river
prop.press_prov <- apply(as.matrix(dist2AQ_prov[,19:29]), 2, function(x) sum(x, na.rm=TRUE))
...
ggplot() + geom_point(data=qsites, aes(prop.press, q)) +
...
totfishall <- arrange(totfishall, -totalfish)
prop.press_prov <- arrange(prop.press_prov, -prop.press)
...
ggplot() + 
  ...
  geom_point(data=prop.press_prov, aes(long, lat, size=prop.press), fill="yellow", shape=21, colour="black") +
  geom_label_repel(data=prop.press_prov, aes(long, lat, label=code), size=3, point.padding = unit(0.5,"lines"))+
...
# match to river mouth coords
riverCWD <- data.frame(code=rivermouths[,3], lat=rivermouths[,2], long = rivermouths[,1], prop.press = prop.press2013)
...
d5 <- as.data.frame(cbind(d5, NSsitecoords$ID))
colnames(d5)[26] <- "ID"

dist2AQ_NS <- join(d5, NSsitecoords, type="left")
```
...
dist2AQ_NS$P11 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V11
dist2AQ_NS$P12 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V12
dist2AQ_NS$P13 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V13
dist2AQ_NS$P14 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V14
dist2AQ_NS$P15 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V15
dist2AQ_NS$P16 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V16
dist2AQ_NS$P17 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V17
dist2AQ_NS$P18 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V18
dist2AQ_NS$P19 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V19
dist2AQ_NS$P20 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V20
dist2AQ_NS$P21 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V21
dist2AQ_NS$P22 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V22
dist2AQ_NS$P23 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V23
dist2AQ_NS$P24 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V24
dist2AQ_NS$P25 <- dist2AQ_NS$Site.Size..HA./dist2AQ_NS$V25


# sum all pressures for total propagule pressure at a river
prop.press_NS <- apply(as.matrix(dist2AQ_NS[,35:59]), 2, function(x) sum(x, na.rm=TRUE))

# match to river mouth coords
riverCWD <- data.frame(code=nsrivers_all[,1], lat=nsrivers_all[,2], long = nsrivers_all[,3], prop.press = prop.press_NS)
