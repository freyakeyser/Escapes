...
dist2AQ$P4 <- dist2AQ$totalfish/as.numeric(dist2AQ$river4)
dist2AQ$P5 <- dist2AQ$totalfish/as.numeric(dist2AQ$river5)
...
plot(NLrast)
text(escapelocs[8,2], escapelocs[8,1])
escapelocs$rivermouth <- c(3,3,3,5,3,2,1,4)

text(riverCWD[,2], riverCWD[,1])
riverCWD$rivermouth <- c(1,2,3,5,5)
...
proportions <- ddply(.data=escapes, .(rivermouth, prop.press),
                     summarize,
                    count = length(Sample.ID))
 
ggplot() + geom_point(data=proportions, aes(log(prop.press), count)) + theme_bw() +
  xlab("Log(Propagule Pressure)") +
