...
## Newfoundland - for low-res generic mapping
```{r}
...
prop.press_all <- data.frame(code=Bsites[,1], lat=rivermouths[,3], long = rivermouths[,2], prop.press = prop.press_all)

write.csv(prop.press_all, "C:/Users/keyserf/Documents/Data/Propagule Pressure_lin_all years_2016-11-21.csv")
...
plot2 <- ggplot() + 
  ...
  scale_size_continuous(name="Propagule pressure=\n\u03A3(total fish/dist)", range = c(0.5,8), limits=c(100,15500), breaks=c(500, 1000, 5000, 10000, 15500))
...
#cropping shapefile to the area and details I need
NLprov <- crop(NL.low, extent(-59.7, -52.4, 46.6, 51.8))
writeOGR(obj=NLprov, dsn="canvec", layer="NLprov", driver="ESRI Shapefile")
NLprov <- readOGR(dsn="C:/Users/keyserf/Documents/R/Escapes/canvec/NLprov.shp", layer="NLprov")
# rm(NL.low)
# rm(NLlab)
...
# match to river mouth coords
prop.press2013 <- data.frame(code=rivermouths[,4], lat=rivermouths[,3], long = rivermouths[,2], prop.press = prop.press2013)
...
prop.press2013
write.csv(prop.press2013, file="C:/Users/keyserf/Documents/Data/R Output/Propagule pressures_lin_2013_sampling_2016-11-21.csv")
```
...
### ESCAPE PRESSURE - Newfoundland all years
## Newfoundland - AQ site coordinates and river mouths
```{r}
...
# Newfoundland Escapes - calculate distance matrix
```{r}
# calculate least-cost distance between siterast (site location) and testrast (escaped fish location)
...
### Newfoundland Escapes - merge with site coords and inventory data
```{r}
...
# Newfoundland Escapes - Calculate propagule pressure for each river. 
```{r}
...
# sum all pressures for total propagule pressure at a river
esc.press_all <- apply(as.matrix(dist2Escape[,24:41]), 2, function(x) sum(x, na.rm=TRUE))

# match to river mouth coords
esc.press_all  <- data.frame(code=rivermouths[,4], lat=rivermouths[,3], long = rivermouths[,2], esc.press = esc.press_all)

write.csv(esc.press_all, "C:/Users/keyserf/Documents/Data/Escape Pressure_lin_all rivers_2002-2012_2016-11-25.csv")

```

# Newfoundland Escapes - plot propagule pressure
```{r}
...
plot2 <- ggplot() + 
  ...
  theme_bw() +  coord_map(xlim=c(-56.7, -54.5), ylim=c(46.82, 48)) +
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.background = element_blank()) +
  scale_size_continuous(name="Escape pressure=\n\u03A3(total escapes/dist)", range = c(0.5,8), limits=c(1,170))
...
qsite_esc <- join(qsites, esc.press_all, type="left")

ggplot() + geom_smooth(data=qsite_esc, aes(esc.press, Q), method="lm", se=TRUE) +
  geom_point(data=qsite_esc, aes(esc.press, Q)) +
  geom_label_repel(data=qsite_esc, aes(esc.press, Q, label=Code)) +
  theme_classic() + 
  theme(panel.background=element_rect(colour="black")) +
  ggtitle("NL 2002-2012")+
...
# Nova Scotia - Merge with inventory data
```{r}
...
## Nova Scotia - q-values vs. propagule pressure
```{r}
ggplot() + 
...
## Nova Scotia - morris escapes vs. propagule pressure
```{r}
