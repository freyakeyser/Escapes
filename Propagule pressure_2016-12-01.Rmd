...
d3 <- costDistance(tr3c, fromCoords = cbind(as.numeric(siterast[,1]), as.numeric(siterast[,2])), toCoords = cbind(as.numeric(sampling2015[,2]), as.numeric(sampling2015[,3])))
...
colnames(d3)[20] <- "ID"
colnames(d3)[1:19] <- as.list(as.character(sampling2015$code))
...
```{r}
dist2AQ_sub <- subset(annual, ReportYear >2013 & ReportYear<2016 & is.na(Lat)==FALSE)
...
```{r}
dist2AQ[,1:19] <- apply(dist2AQ[,1:19], 2, function(x) as.character(x))
dist2AQ[,1:19] <- apply(dist2AQ[,1:19], 2, function(x) as.numeric(x))
...
prop.press_all <- apply(as.matrix(dist2AQ[,24:42]), 2, function(x) sum(x, na.rm=TRUE))
