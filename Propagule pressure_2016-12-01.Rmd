...
names(reportlocs) <-c("River", "latfinal", "longfinal")
...
sitecoords_atl <- unique(sitecoords_atl)
...
dev.off()
plot(NAm, xlim=c(-65, -64), ylim=c(44,45))
points(sitecoords_atl[sitecoords_atl$Site.ID%in%c(74) & sitecoords_atl$prov=="NS", 4:3]$Long, sitecoords_atl[sitecoords_atl$Site.ID%in%c(74) & sitecoords_atl$prov=="NS", 4:3]$Lat) <- c()
plot(NAm, xlim=c(-61, -60.5), ylim=c(46,46.5))
sitecoords_atl[sitecoords_atl$Site.ID%in%c(193) & sitecoords_atl$prov=="NS", 4:3] <- c()

plot(NAm, xlim=c(-61, -60.5), ylim=c(46,46.5))
sitecoords_atl[sitecoords_atl$Site.ID%in%c(16,34,54,168,350,491,14,44,49,156) & sitecoords_atl$prov=="NB", 4:3] <- c()

plot(NAm, xlim=c(-61.5, -60), ylim=c(46,46.8))
reportlocs[reportlocs$River %in% c("Clyburn", "Margaree"), 4:3] <- c()

plot(NAm, xlim=c(-65, -64), ylim=c(44, 45))
reportlocs[reportlocs$River == "Lahave", 4:3] <- c()
...
dev.off()
plot(ATLrast2, xlim=c(-58, -57), ylim=c(47, 48))
points(reportlocs$longfinal[reportlocs$River=="Grandy Brook"], reportlocs$latfinal[reportlocs$River=="Grandy Brook"])
...
datl_full2 <- costDistance(tratlc, fromCoords = cbind(as.numeric(sitecoords_atl[1:10,]$Long), as.numeric(sitecoords_atl[1:10,]$Lat)), toCoords = cbind(as.numeric(reportlocs[1:10,]$longfinal), as.numeric(reportlocs[1:10,]$latfinal)))
...
colnames(datl_full)[73] <- "Site.ID"
colnames(datl_full)[74] <- "prov"
...
dist2AQ_sub <- subset(dist2AQ_sub, Year>2004 & Year < 2016)
...
dist2AQ[,1:72] <- apply(dist2AQ[,1:72], 2, function(x) as.character(x))
dist2AQ[,1:72] <- apply(dist2AQ[,1:72], 2, function(x) as.numeric(x))
...
# For each AQ site, caclulate total fish/distance from river
## original calculation
test <- c(dist2AQ$totalfish/dist2AQ[,1:72])
dist2AQ <- cbind(dist2AQ, test)

# sum all pressures for total propagule pressure at a river
prop.press_allreports <- apply(as.matrix(dist2AQ[,79:150]), 2, function(x) sum(x, na.rm=TRUE))

# for each year
prop.pressall_2005 <- apply(as.matrix(dist2AQ[dist2AQ$Year==2005, 79:150]), 2, function(x) sum(x, na.rm=TRUE))
prop.pressall_2006 <- apply(as.matrix(dist2AQ[dist2AQ$Year==2006, 79:150]), 2, function(x) sum(x, na.rm=TRUE))
prop.pressall_2007 <- apply(as.matrix(dist2AQ[dist2AQ$Year==2007, 79:150]), 2, function(x) sum(x, na.rm=TRUE))
prop.pressall_2008 <- apply(as.matrix(dist2AQ[dist2AQ$Year==2008, 79:150]), 2, function(x) sum(x, na.rm=TRUE))
prop.pressall_2009 <- apply(as.matrix(dist2AQ[dist2AQ$Year==2009,  79:150]), 2, function(x) sum(x, na.rm=TRUE))
prop.pressall_2010<- apply(as.matrix(dist2AQ[dist2AQ$Year==2010,  79:150]), 2, function(x) sum(x, na.rm=TRUE))
prop.pressall_2011<- apply(as.matrix(dist2AQ[dist2AQ$Year==2011,  79:150]), 2, function(x) sum(x, na.rm=TRUE))
prop.pressall_2012<- apply(as.matrix(dist2AQ[dist2AQ$Year==2012,  79:150]), 2, function(x) sum(x, na.rm=TRUE))
prop.pressall_2013<- apply(as.matrix(dist2AQ[dist2AQ$Year==2013,  79:150]), 2, function(x) sum(x, na.rm=TRUE))
prop.pressall_2014<- apply(as.matrix(dist2AQ[dist2AQ$Year==2014,  79:150]), 2, function(x) sum(x, na.rm=TRUE))
prop.pressall_2015<- apply(as.matrix(dist2AQ[dist2AQ$Year==2015,  79:150]), 2, function(x) sum(x, na.rm=TRUE))


prop.press_all_byyear <- data.frame(prop.press=c(prop.pressall_2005, prop.pressall_2006, prop.pressall_2007, prop.pressall_2008, prop.pressall_2009, prop.pressall_2010, prop.pressall_2011, prop.pressall_2012, prop.pressall_2013, prop.pressall_2014, prop.pressall_2015), Year=rep(2005:2015, each=72), River=rep(reportlocs$River, 11), lat=rep(reportlocs$latfinal, 11), long=rep(reportlocs$longfinal, 11))
...
detections <- ddply(.data=timereports[timereports$Year.adj > 2004 & fullreports$Year.adj < 2016,], .(River),
                    summarize,
                    detect = max(detected))
...
ggplot() + geom_point(data=prop.press_avg_reports, aes(as.character(River), sProp.press))
...
alldetections <- ddply(.data=timereports, .(River, longfinal, latfinal, detected),
                    summarize,
...
ggplot() + 
  ...
  # annotate(geom="rect", xmin=-64, xmax=-59.5, ymin=43.2, ymax= 44, fill="grey") +
  # annotate(geom="rect", xmin=-61, xmax=-59.5, ymin=44, ymax= 45, fill="grey") + 
 geom_point(data=alldetections[alldetections$detected==0.5 | alldetections$detected==0,], aes(longfinal, latfinal, fill=as.factor(detected)), shape=21, size=2) +
  geom_point(data=alldetections[alldetections$totalfarmed>0,], aes(longfinal, latfinal, size=totalfarmed), shape=21, fill="red2") + 
...
names(reportlocs) <- c("X.1", "X", "River", "Lat", "Long")

## for all reports
calcAQpress(AQsites = sitecoords_atl[is.na(sitecoords_atl$Long)=="FALSE",], inventory = inventory1[is.na(inventory1$Long)=="FALSE",], rivercoords = reportlocs, dir="C:/Users/keyserf/Documents/Data/R output")
