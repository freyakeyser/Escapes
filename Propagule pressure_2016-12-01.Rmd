...
reportlocs[reportlocs$River=="Bay Du Nord", 3] <- -55.4175
reportlocs[reportlocs$River=="Bay Du Nord", 2] <- 47.71868
...
reportlocs[reportlocs$River=="Tickle Harbour", 3:2] <- c(-55.13155, 47.67407)
...
fullreports[fullreports$Year.adj==2016 & is.na(fullreports$detected)==TRUE,]$detected <- 0

fullreports <- fullreports[is.na(fullreports$River)==FALSE,]
...
ggplot() + #geom_spatial(data=NB.low, aes(fill=ctry_en)) + ### canvec spatialpolygonsdataframe
 ...
 # geom_spatial(data=NS.low, aes(fill=ctry_en)) + ### canvec spatialpolygonsdataframe
  ...
  # annotate(geom="rect", xmin=-64, xmax=-59.5, ymin=43.2, ymax= 44, fill="grey") +
  # annotate(geom="rect", xmin=-61, xmax=-59.5, ymin=44, ymax= 45, fill="grey") + 
  geom_point(data=fullreports, aes(longfinal1, latfinal1, colour=as.factor(detected))) + 
  coord_map() + 
  theme_classic() 
```
...
```{r}
fullreports[is.na(fullreports$Year.adj),]$Year

fullreports[is.na(fullreports$Year.adj),]$Year.adj <- c(1996, 1990, 1989, NA, 2005,
                                                        1996, 1996, NA, 1999, NA,
                                                        1996, NA, 1994, NA, 1998,
...
calcAQpress(AQsites = sitecoords_atl, rivercoords = reportlocs)
...
timereports$River <- gsub("(",".",timereports$River, fixed=TRUE)
timereports$River <- gsub(")",".",timereports$River, fixed=TRUE)
timereports$River <- gsub("'",".",timereports$River, fixed=TRUE)


timereports[timereports$River=="Bay.du.Nord.River",]$River<- "Bay.Du.Nord"
timereports[timereports$River=="Bay.Du.Nord",]$latfinal1 <- 47.70501 
timereports[timereports$River=="Bay.Du.Nord",]$longfinal1 <- -55.42238

detections$River <- gsub("\\s",".",detections$River)
detections$River <- gsub("(",".",detections$River, fixed=TRUE)
detections$River <- gsub(")",".",detections$River, fixed=TRUE)
detections$River <- gsub("'",".",detections$River, fixed=TRUE)
...
require(car)
require(MASS)
# poisson <- fitdistr(totals$total, "Poisson")
# qqp(totals$total, "pois", poisson$estimate)

# qqp(totals$total, "norm")
# qqp(totals$total, "lnorm")

nbinom <- fitdistr(totals$total, "Negative Binomial")
qqp(totals$total, "nbinom", size = nbinom$estimate[[1]], mu = nbinom$estimate[[2]])
...
alldetections <- ddply(.data=timereports2, .(River, longfinal1, latfinal1, detected),
                    summarise,
