...
siterast <- as.matrix(cbind(sitecoords_all$Long, sitecoords_all$Lat))

# 
# require(marmap)
# atl <- getNOAA.bathy(lon1=-68, lon2=-59, lat1=42, lat2=48, resolution=0.5)
# 
# plot.bathy(atl)
...
# Change values in raster so that land is impassable (i.e. distances would be massive).
MARrast@data@values <- ifelse(is.na(MARrast@data@values)=="FALSE", 100000, 1)

plot(MARrast)

# create transition layer for raster (likelihood of presence within cell). 
trmar <- transition(1/MARrast, transitionFunction=mean, directions=8)
trmarc <- geoCorrection(trmar, type="c")

# rastmap <- as.data.frame(cbind(coordinates(MARrast), MARrast@data@values))
# 
# names(rastmap)
# 
# maritimes$roundedlong <- round(maritimes$Long, 4)
# maritimes$roundedlat <- round(maritimes$Lat, 4)
# rastmap$x <- round(rastmap$x, 4)
# rastmap$y <- round(rastmap$y, 4)
# 
# rastmap <- join(rastmap, maritimes, type="left")
...
dmar <- costDistance(trmarc, fromCoords = cbind(as.numeric(sitecoords_all$Long), as.numeric(sitecoords_all$Lat)), toCoords = cbind(as.numeric(maritimes$Long), as.numeric(maritimes$Lat)))

dmar <- as.data.frame(dmar)

# dmar_named <- dmar
# 
# colnames(dmar_named) <- as.character(maritimes$Name)
# 
# dmar_named <- dmar_named[ ,(dmar_named[1, ] < 10000000)]
# 
# View(dmar_named)
# 
# dmar2 <- dmar
# 
# colnames(dmar2) <- maritimes$Name
# 
# View(dmar2)
# 
# str(dmar)

sitecoords_all<- read.csv("C:/Users/keyserf/Documents/Data/Rivers/sitecoords_all.csv")
maritimes<- read.csv("C:/Users/keyserf/Documents/Data/Rivers/maritimes.csv")


# Identify and remove "bad" rivers from maritimes DF
test <- c(dmar[1,] < 10000000) # if less than 10 mill, true
str(test)
test <- data.frame(test=test, val = 1:387)
test$var <- paste0("V", test$val)
test <- subset(test, test=="TRUE")
test

namesyes <- test$var             
               
maritimes$var <- paste0("V", 1:387)

maritimes2 <- join(maritimes, test, type="left")
maritimes2 <- subset(maritimes2, test=="TRUE")

maritimes2$Name <- as.factor(as.character(maritimes2$Name))
maritimes2 <- arrange(maritimes2, Long, Lat)
maritimesrast <- cbind(maritimes2$Long, maritimes2$Lat)
maritimesrast <- as.matrix(maritimesrast)

# make sitecoords into matrix? --> didn't help
x <- sitecoords2$long
y <- sitecoords2$lat
xy2 <- cbind(x, y)
siterast2 <- as.matrix(xy2)
...
# Use maritimes rivers from previous analyses instead. Fix above tomorrow.
marrivers <- rbind(nsrivers, nbrivers2) 

# Re-run with only "good" maritimes rivers (bad values are still appearing... why? Are columns re-ordering somehow?)
dmar <- costDistance(trmarc, fromCoords = cbind(as.numeric(siterast[,1]), as.numeric(siterast[,2])), toCoords = cbind(as.numeric(maritimesrast$long), as.numeric(marrivers$lat)))

dmar <- as.data.frame(cbind(dmar, sitecoords_all$Site.ID))



colnames(dmar)[1:38] <- as.list(as.character(marrivers$code))
colnames(dmar)[39] <- "Site.ID"
...
dist2AQ <- join(dmar, dist2AQ_sub, type="left")
dist2AQ <- subset(dist2AQ, is.na(totalfish)==FALSE)


dist2AQ[,1:38] <- apply(dist2AQ[,1:38], 2, function(x) as.character(x))
dist2AQ[,1:38] <- apply(dist2AQ[,1:38], 2, function(x) as.numeric(x))

# For each AQ site, caclulate total fish/distance from river
## original calculation
test <- c(dist2AQ$totalfish/dist2AQ[,1:38])
dist2AQ <- cbind(dist2AQ, test)

# sum all pressures for total propagule pressure at a river
prop.press_MILLIONRIVERS <- apply(as.matrix(dist2AQ[,43:80]), 2, function(x) sum(x, na.rm=TRUE))

# match to river mouth coords
prop.press_MILLIONRIVERS <- data.frame(code = marrivers$code, Lat=marrivers$lat, Long = marrivers$long, prop.press = prop.press_MILLIONRIVERS)
...
dist2AQ[,c(228,250,251,252,253,481)]

plot(MARrast)
text(maritimes$Long, maritimes$Lat)
