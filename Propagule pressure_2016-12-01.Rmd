...
# for SFA 10 and 11 
sched <- read.csv("C:/Users/keyserf/Documents/Data/Rivers/NL Scheduled Coords.csv")
...
qvals <- rbind(qvals, prop.press_NL_q)
...
ggplot() + 
  #geom_smooth(data=qvals, aes(prop.press, q), colour="black", method="lm", se=TRUE) +
  geom_point(data=qvals, aes(prop.press, q, fill=prov, shape=prov), colour="black", size=3, alpha=0.5) +
  geom_line(data=qvals, aes(prop.press, V7)) +
  geom_line(data=qvals, aes(prop.press, V8), lty="dashed") +
  geom_line(data=qvals, aes(prop.press, V9), lty="dashed") +
  #geom_label_repel(data=qvals, aes(prop.press, q, label=code)) +
  theme_classic() + 
  theme(panel.background=element_rect(colour="black")) +
  scale_shape_manual(values=c(21,22,23), labels=c("NB (2002-2012)", "NL (2005-2012)", "NS (2002-2012)"))+
  scale_fill_discrete(labels=c("NB (2002-2012)", "NL (2005-2012)", "NS (2002-2012)"))+
  xlab("Propagule pressure") +
  ylab("Q value") +
  geom_text_repel(data=qvals[qvals$out=="out",], aes(prop.press, q, label=code), size=3) +
...
# try to add in distance for tob and nsh and nic too
```
